...
app.use((req, res) => {
    ...
    if (req.path.startsWith('/play/')) {
        ...
        if (videoId) {
            ...
            const playTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist}` : qTitle) : `Dengarkan Musik - Alfath`;
            const playDesc = `Dengarkan ${qTitle || 'lagu favoritmu'} di Alfath Web Music Player`;
    ...
    if (req.path.startsWith('/artist/')) {
        ...
        if (artistId) {
            ...
            const pageTitle = qName ? `${qName} (Artist) - Alfath` : `Artist - Alfath`;
            const pageDesc = qName ? `Dengarkan lagu & album terbaik dari ${qName} di Alfath` : `Dengarkan lagu & album dari artist favoritmu di Alfath`;
    ...
    if (req.path.startsWith('/album/')) {
        ...
        if (albumId) {
            ...
            const pageTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist} (Album) - Alfath` : `${qTitle} (Album) - Alfath`) : `Album - Alfath`;
            const pageDesc = qTitle ? `Dengarkan album ${qTitle} di Alfath` : `Dengarkan album favoritmu di Alfath`;
            const coverUrl = qCover || `https://www.gobox.my.id/file/R0ym4wqfznmp.png`;
