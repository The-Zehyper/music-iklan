...
async function getTranscribe(urlOrVideoId) {
    ...
    try {
        const uploadRes = await axios.post('https://api.assemblyai.com/v2/upload',
            fs.createReadStream(mp3Path), { headers: { 'Authorization': ASSEMBLYAI_KEY }, maxBodyLength: Infinity, timeout: 60000 }
        );
...
const handler = async (req, res) => {
    ...
    if (!url) return res.status(400).json({ creator: 'Alfath', status: false, message: 'Parameter url atau id diperlukan' });
    ...
    try {
        ...
        return res.status(500).json({ creator: 'Alfath', status: false, message: err.message });
    }
};
