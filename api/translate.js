...
const handler = async (req, res) => {
    ...
    try {
        if (lines && Array.isArray(lines)) {
            ...
            return res.status(200).json({ status: true, result: { lines: translatedLines, creator: 'Alfath' } });
        } else if (text) {
            const translatedText = await translateText(text, targetLang);
            return res.status(200).json({ status: true, result: { original: text, translated: translatedText, targetLang, creator: 'Alfath' } });
        }
    } catch(e) {
